...
async def reset_authkey(request: Request, token: str = Depends(get_token)):
    ...
    if role != "ADMIN" and (role != "3PL"):
        logger.error("Unauthorized role for resetting auth key: ", role)
        raise HTTPException("Unauthorized role for resetting auth key")
    if role == "ADMIN":
        provider = body['3pl']
...
async def view_authkey(request: Request, token: str = Depends(get_token)):
    ...
    if role != "ADMIN" and role != "3PL":
        logger.error("Unauthorized role for viewing auth key: ", role)
        raise HTTPException("Unauthorized role for viewing auth key")
    if role == "ADMIN":
        provider = body['3pl']

...
def log_db_self_table(res, operation):
    response = res['ResponseMetadata']['HTTPStatusCode']
    logger.info(f'Operation performed is {operation} with response code {response}')
...
class DBHelper:
    ...
    def insert_lead(self, lead_hash: str, lead_provider: str, response: str):
        ...
        log_db_self_table(res, "put_item")
    ...
    def check_duplicate_api_call(self, lead_hash: str, lead_provider: str):
        ...
        log_db_self_table(res, "get_item")
        item = res.get('Item')
        if not item:
            return {
                "Duplicate_Api_Call": {
    ...
    def check_duplicate_lead(self, email: str, phone: str, last_name: str, make: str, model: str):
        ...
        log_db_self_table(phone_attached_leads, "query")

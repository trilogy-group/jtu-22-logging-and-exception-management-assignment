...
def logger_info_db_operation(responseCode, context:str, operation:str):
    if responseCode == 200:
...
class DBHelper:
    ...
    def insert_lead(self, lead_hash: str, lead_provider: str, response: str):
        ...
        logger_info_db_operation(res['ResponseMetadata']['HTTPStatusCode'], 'insert_lead', 'put_item')
    ...
    def insert_oem_lead(self, uuid: str, make: str, model: str, date: str, email: str, phone: str, last_name: str,
        ...
        res = self.table.put_item(Item=item)
        logger_info_db_operation(res['ResponseMetadata']['HTTPStatusCode'], 'inset_oem_lead', 'put_item')
    ...
    def check_duplicate_api_call(self, lead_hash: str, lead_provider: str):
        ...
        item = res.get('Item')
        logger_info_db_operation(res['ResponseMetadata']['HTTPStatusCode'], 'check_duplicate_api_call', 'get')
        if not item:
            return {
                "Duplicate_Api_Call": {
                    "status": False,
    ...
    def accepted_lead_not_sent_for_oem(self, oem: str, date: str):
        ...
        logger_info_db_operation(res['ResponseMetadata']['HTTPStatusCode'], 'accepted_lead_not_sent_for_oem', 'query')
    ...
    def update_lead_sent_status(self, uuid: str, oem: str, make: str, model: str):
        ...
        item['gsisk'] = "1#0"
        res = self.table.put_item(Item=item)
        logger_info_db_operation(res['ResponseMetadata']['HTTPStatusCode'], 'update_lead_sent_status', 'put_item')
        return True
    ...
    def get_make_model_filter_status(self, oem: str):
        ...
        logger_info_db_operation(res['ResponseMetadata']['HTTPStatusCode'], 'get_make_filter_status', 'get_item')
    ...
    def verify_api_key(self, apikey: str):
        ...
        item = res.get('Items', [])
        logger_info_db_operation(res['ResponseMetadata']['HTTPStatusCode'], 'verify_api_key', 'get')
        if len(item) == 0:
            return False
        return True
    ...
    def get_auth_key(self, username: str):
        ...
        logger_info_db_operation(res['ResponseMetadata']['HTTPStatusCode'], 'get_auth_key', 'query')
        item = res['Items']
        if len(item) == 0:
            return None
        return item[0]['sk']
    ...
    def set_auth_key(self, username: str):
        ...
        logger_info_db_operation(res['ResponseMetadata']['HTTPStatusCode'], 'set_auth_key', 'put_item')
        return apikey
    ...
    def register_3PL(self, username: str):
        ...
        logger_info_db_operation(res['ResponseMetadata']['HTTPStatusCode'], 'register_3PL', 'query')
        item = res.get('Items', [])
        if len(item):
            return None
    ...
    def set_make_model_oem(self, oem: str, make_model: str):
        ...
        res = self.table.put_item(Item=item)
        logger_info_db_operation(res['ResponseMetadata']['HTTPStatusCode'], 'set_make_model_oem', 'put_item')
    ...
    def fetch_oem_data(self, oem, parallel=False):
        ...
        logger_info_db_operation(res['ResponseMetadata']['HTTPStatusCode'], 'fetch_oem_data', 'get_item')
        if 'Item' not in res:
            return {}
        if parallel:
            return {
    ...
    def create_new_oem(self, oem: str, make_model: str, threshold: str):
        ...
        logger_info_db_operation(res['ResponseMetadata']['HTTPStatusCode'], 'create_new_open', 'put_item')
    ...
    def delete_oem(self, oem: str):
        ...
        logger_info_db_operation(res['ResponseMetadata']['HTTPStatusCode'], 'delete_oem', 'delete_item')
    ...
    def delete_3PL(self, username: str):
        ...
        if authkey:
            ...
            logger_info_db_operation(res['ResponseMetadata']['HTTPStatusCode'], 'delete_3PL', 'delete_item')
    ...
    def set_oem_threshold(self, oem: str, threshold: str):
        ...
        res = self.table.put_item(Item=item)
        logger_info_db_operation(res['ResponseMetadata']['HTTPStatusCode'], 'set_oem_threshold', 'put_item')
        return {
    ...
    def get_dealer_data(self, dealer_code: str, oem: str):
        ...
        logger_info_db_operation(res['ResponseMetadata']['HTTPStatusCode'], 'get_dealer_data', 'query')
        res = res['Items']
        if len(res) == 0:
            return {}
        res = res[0]
    ...
    def insert_customer_lead(self, uuid: str, email: str, phone: str, last_name: str, make: str, model: str):
        ...
        res = self.table.put_item(Item=item)
        logger_info_db_operation(res['ResponseMetadata']['HTTPStatusCode'], 'insert_customer_lead', 'put_item')
    ...
    def lead_exists(self, uuid: str, make: str, model: str):
        ...
        if self.get_make_model_filter_status(make):
            ...
            logger_info_db_operation(res['ResponseMetadata']['HTTPStatusCode'], 'lead_exists', 'query')
            if len(res['Items']):
                lead_exist = True
        else:
            res = self.table.query(
    ...
    def check_duplicate_lead(self, email: str, phone: str, last_name: str, make: str, model: str):
        ...
        logger_info_db_operation(email_attached_leads['ResponseMetadata']['HTTPStatusCode'], 'check_duplicate_lead.email_attached_leads', 'query')
        phone_attached_leads = self.table.query(
            IndexName='gsi1-index',
            KeyConditionExpression=Key('gsipk1').eq(f"{phone}#{last_name}")
        )
        logger_info_db_operation(phone_attached_leads['ResponseMetadata']['HTTPStatusCode'], 'check_duplicate_lead.phone_attached_leads', 'query')
    ...
    def get_api_key_author(self, apikey):
        ...
        logger_info_db_operation(res['ResponseMetadata']['HTTPStatusCode'], 'get_api_key_author', 'query')
        item = res.get('Items', [])
        if len(item) == 0:
            return "unknown"
    ...
    def update_lead_conversion(self, lead_uuid: str, oem: str, converted: int):
        ...
        logger_info_db_operation(res['ResponseMetadata']['HTTPStatusCode'], 'update_lead_conversion', 'query')
        items = res.get('Items')
        if len(items) == 0:
            return False, {}
        item = items[0]
        ...
        res = self.table.put_item(Item=item)
        logger_info_db_operation(res['ResponseMetadata']['HTTPStatusCode'], 'update_lead_conversion', 'put_item')
        return True, item

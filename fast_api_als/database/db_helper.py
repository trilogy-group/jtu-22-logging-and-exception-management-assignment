...
def log(res, info):
    ...
    logging.info(f"[DB] {info} ,reponse code: {response_code}")
...
class DBHelper:
    ...
    def insert_lead(self, lead_hash: str, lead_provider: str, response: str):
        ...
        log(res,"Lead Insertion, Lead hash -> {lead_hash} ")
    ...
    def insert_oem_lead(self, uuid: str, make: str, model: str, date: str, email: str, phone: str, last_name: str,
        ...
        res = self.table.put_item(Item=item)
        log(res,"Lead OEM Insertion, UUid -> {uuid} ")
    ...
    def check_duplicate_api_call(self, lead_hash: str, lead_provider: str):
        ...
        log(res,"Lead Get, Lead_hash -> {lead_hash}")
        item = res.get('Item')
        if not item:
            return {
                "Duplicate_Api_Call": {
    ...
    def accepted_lead_not_sent_for_oem(self, oem: str, date: str):
        ...
        log(res,f"Accepted Lead Not Sent For OEM Query, OEM -> {oem}")
    ...
    def update_lead_sent_status(self, uuid: str, oem: str, make: str, model: str):
        ...
        log(res,f"Lead Get, UUID -> {uuid} , OEM -> {oem}")
        item = res['Item']
        if not item:
            return False
        item['gsisk'] = "1#0"
        res = self.table.put_item(Item=item)
        log(res,f"Lead Update, UUid -> {uuid},OEM -> {oem}")
        return True
    ...
    def get_make_model_filter_status(self, oem: str):
        ...
        log(res,f"OEM Get Query, OEM -> {oem}")
    ...
    def verify_api_key(self, apikey: str):
        ...
        log(res,f"Api Key Match Query, ApiKey -> {apikey}")
        item = res.get('Items', [])
        if len(item) == 0:
            return False
        return True
    ...
    def get_auth_key(self, username: str):
        ...
        log(res,f"Get auth query, username -> {username}")
        item = res['Items']
        if len(item) == 0:
            return None
        return item[0]['sk']
    ...
    def set_auth_key(self, username: str):
        ...
        log(res,f"Set auth key query, username -> {username}, apikey -> {apikey}")
        return apikey
    ...
    def register_3PL(self, username: str):
        ...
        log(res,f"Get query, username -> {username}")
        item = res.get('Items', [])
        if len(item):
            return None
    ...
    def set_make_model_oem(self, oem: str, make_model: str):
        ...
        res = self.table.put_item(Item=item)
        log(res,f"Put Make model query, oem -> {oem}, MakeModel -> {make_model}")
    ...
    def fetch_oem_data(self, oem, parallel=False):
        ...
        log(res,f"Fetch OEM Query, oem -> {oem}")
        if 'Item' not in res:
            return {}
        if parallel:
            return {
    ...
    def create_new_oem(self, oem: str, make_model: str, threshold: str):
        ...
        log(res,f"Put Oem, oem -> {oem}, make_model={make_model}, threshold={threshold}")
    ...
    def delete_3PL(self, username: str):
        ...
        log(res,f"Delete 3PL, username -> {username}")
    ...
    def set_oem_threshold(self, oem: str, threshold: str):
        ...
        res = self.table.put_item(Item=item)
        log(res,f"Put Oem Threshold, oem -> {oem}, threshold -> {threshold}")
        return {
    ...
    def fetch_nearest_dealer(self, oem: str, lat: str, lon: str):
        ...
        log(res,f"Nearest Dealer Find Query, oem -> {oem}, lat -> {lat}, long -> {long}")
        if len(res) == 0:
            return {}
        res = res[0]
        dealer = {
    ...
    def get_dealer_data(self, dealer_code: str, oem: str):
        ...
        log(res,f"Get Dealer Query, dealer_code -> {dealer_code} , oem -> {oem}")
        res = res['Items']
        if len(res) == 0:
            return {}
        res = res[0]
    ...
    def insert_customer_lead(self, uuid: str, email: str, phone: str, last_name: str, make: str, model: str):
        ...
        res = self.table.put_item(Item=item)
        log(res,f"Put Lead Query, uuid -> {uuid} , email -> {email}")
    ...
    def lead_exists(self, uuid: str, make: str, model: str):
        ...
        if self.get_make_model_filter_status(make):
            ...
            log(res,f"Get Lead Query, uuid -> {uuid} , make -> {make}, model -> {model}")
            if len(res['Items']):
                lead_exist = True
        else:
            res = self.table.query(
            ...
            log(res,f"Get Lead Query, uuid -> {uuid} , make -> {make}")
            if len(res['Items']):
                lead_exist = True
        return lead_exist
    ...
    def get_api_key_author(self, apikey):
        ...
        log(res,f"Get Apikey author query, Apikey -> {apikey} ")
        item = res.get('Items', [])
        if len(item) == 0:
            return "unknown"
    ...
    def update_lead_conversion(self, lead_uuid: str, oem: str, converted: int):
        ...
        log(res,f"Get Lead Query, uuid -> {lead_uuid} , oem -> {oem}")
        items = res.get('Items')
        if len(items) == 0:
            return False, {}
        item = items[0]

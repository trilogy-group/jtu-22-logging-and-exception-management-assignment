...
async def reset_authkey(request: Request, token: str = Depends(get_token)):
    ...
    if role != "ADMIN" and (role != "3PL"):
        raise HTTPException(status_code=401, detail="fn:reset_authkey: Unauthorized to reset_authkey")
    if role == "ADMIN":
        provider = body['3pl']
...
async def view_authkey(request: Request, token: str = Depends(get_token)):
    ...
    if role != "ADMIN" and role != "3PL":
        raise HTTPException(status_code=401, detail="fn:view_authkey: Unauthorized to view_authkey")
    if role == "ADMIN":
        provider = body['3pl']

...
def get_quicksight_data(lead_uuid, item):
    """
            ...
            Returns:
                S3 data
    """
    logger.info(f"Initiated S3 data conversion for item with lead UUID {lead_uuid}")
    data = {
        "lead_hash": lead_uuid,
    ...
    logger.info(f"S3 data conversion of item with lead UUID {lead_uuid} successful")
    return data, f"{item['make']}/1_{int(time.time())}_{lead_uuid}"


@router.post("/conversion")
async def submit(file: Request, token: str = Depends(get_token)):
    logger.info("Lead conversion request")
    body = await file.body()
    body = json.loads(str(body, 'utf-8'))
    if 'lead_uuid' not in body or 'converted' not in body:
        logger.info("Lead conversion request terminated due to missing info")
    ...
    if is_updated:
        ...
        logger.info(f"Lead conversion is successful for item with uuid {lead_uuid}")
        return {
            "status_code": status.HTTP_200_OK,
            "message": "Lead Conversion Status Update"
        }
    else:
        logger.error{f"Lead conversion request terminated: item with uuid {lead_uuid} is already updated"}

...
def logHTTP(responseCode, fn, op):
    responseCode=str(responseCode)
    message = f'In {fn} : self.table.{op} ended with response code {responseCode}.'
    if responseCode[0]==2 or responseCode[0]==1 :
...
class DBHelper:
    ...
    def insert_lead(self, lead_hash: str, lead_provider: str, response: str):
        ...
        logHTTP(res['ResponseMetadata']['HTTPStatusCode'],'insert_lead','put_item')
    ...
    def insert_oem_lead(self, uuid: str, make: str, model: str, date: str, email: str, phone: str, last_name: str,
        ...
        res = self.table.put_item(Item=item)
        logHTTP(res['ResponseMetadata']['HTTPStatusCode'],'insert_oem_lead','put_item')
    ...
    def check_duplicate_api_call(self, lead_hash: str, lead_provider: str):
        ...
        logHTTP(res['ResponseMetadata']['HTTPStatusCode'],'check_duplicate_api_call','get_item')
        item = res.get('Item')
        if not item:
            return {
                "Duplicate_Api_Call": {
    ...
    def accepted_lead_not_sent_for_oem(self, oem: str, date: str):
        ...
        logHTTP(res['ResponseMetadata']['HTTPStatusCode'],'accepted_lead_not_sent_for_oem','query')
    ...
    def update_lead_sent_status(self, uuid: str, oem: str, make: str, model: str):
        ...
        logHTTP(res['ResponseMetadata']['HTTPStatusCode'],'update_lead_sent_status','get_item')
        item = res['Item']
        if not item:
            return False
        item['gsisk'] = "1#0"
        res = self.table.put_item(Item=item)
        logHTTP(res['ResponseMetadata']['HTTPStatusCode'],'update_lead_sent_status','put_item')
        return True
    ...
    def get_make_model_filter_status(self, oem: str):
        ...
        logHTTP(res['ResponseMetadata']['HTTPStatusCode'],'get_make_model_filter_status','get_item')
    ...
    def verify_api_key(self, apikey: str):
        ...
        logHTTP(res['ResponseMetadata']['HTTPStatusCode'],'verify_api_key','query')
        item = res.get('Items', [])
        if len(item) == 0:
            return False
        return True
    ...
    def get_auth_key(self, username: str):
        ...
        logHTTP(res['ResponseMetadata']['HTTPStatusCode'],'get_auth_key','query')
        item = res['Items']
        if len(item) == 0:
            return None
        return item[0]['sk']
    ...
    def set_auth_key(self, username: str):
        ...
        logHTTP(res['ResponseMetadata']['HTTPStatusCode'],'set_auth_key','put_item')
        return apikey
    ...
    def register_3PL(self, username: str):
        ...
        logHTTP(res['ResponseMetadata']['HTTPStatusCode'],'register_3PL','query')
        item = res.get('Items', [])
        if len(item):
            return None
    ...
    def set_make_model_oem(self, oem: str, make_model: str):
        ...
        res = self.table.put_item(Item=item)
        logHTTP(res['ResponseMetadata']['HTTPStatusCode'],'set_make_model_oem','put_item')
    ...
    def fetch_oem_data(self, oem, parallel=False):
        ...
        logHTTP(res['ResponseMetadata']['HTTPStatusCode'],'fetch_oem_data','get_item')
        if 'Item' not in res:
            return {}
        if parallel:
            return {
    ...
    def create_new_oem(self, oem: str, make_model: str, threshold: str):
        ...
        logHTTP(res['ResponseMetadata']['HTTPStatusCode'],'create_new_oem','put_item')
    ...
    def delete_oem(self, oem: str):
        ...
        logHTTP(res['ResponseMetadata']['HTTPStatusCode'],'delete_oem','delete_item')
    ...
    def delete_3PL(self, username: str):
        ...
        if authkey:
            ...
            logHTTP(res['ResponseMetadata']['HTTPStatusCode'],'delete_3PL','delete_item')
    ...
    def set_oem_threshold(self, oem: str, threshold: str):
        ...
        res = self.table.put_item(Item=item)
        logHTTP(res['ResponseMetadata']['HTTPStatusCode'],'set_oem_threshold','put_item')
        return {
    ...
    def insert_customer_lead(self, uuid: str, email: str, phone: str, last_name: str, make: str, model: str):
        ...
        res = self.table.put_item(Item=item)
        logHTTP(res['ResponseMetadata']['HTTPStatusCode'],'insert_customer_lead','put_item')
    ...
    def lead_exists(self, uuid: str, make: str, model: str):
        ...
        if self.get_make_model_filter_status(make):
            ...
            logHTTP(res['ResponseMetadata']['HTTPStatusCode'],'lead_exists','query')
            if len(res['Items']):
                lead_exist = True
        else:
            res = self.table.query(
    ...
    def check_duplicate_lead(self, email: str, phone: str, last_name: str, make: str, model: str):
        ...
        res=email_attached_leads
        logHTTP(res['ResponseMetadata']['HTTPStatusCode'],'check_duplicate_lead','query')
        phone_attached_leads = self.table.query(
            IndexName='gsi1-index',
            KeyConditionExpression=Key('gsipk1').eq(f"{phone}#{last_name}")
        )
        res=phone_attached_leads
        logHTTP(res['ResponseMetadata']['HTTPStatusCode'],'check_duplicate_lead','query')
    ...
    def get_api_key_author(self, apikey):
        ...
        logHTTP(res['ResponseMetadata']['HTTPStatusCode'],'get_api_key_author','query')
        item = res.get('Items', [])
        if len(item) == 0:
            return "unknown"
    ...
    def update_lead_conversion(self, lead_uuid: str, oem: str, converted: int):
        ...
        logHTTP(res['ResponseMetadata']['HTTPStatusCode'],'update_lead_conversion','query')
        items = res.get('Items')
        if len(items) == 0:
            return False, {}
        item = items[0]
        ...
        res = self.table.put_item(Item=item)
        logHTTP(res['ResponseMetadata']['HTTPStatusCode'],'update_lead_conversion','put_item')
        return True, item


def verify_response(response_code):
    if not response_code == 200:
        logging.info("The request succeeded.")
    else:
        logging.info(f'Request ended with response code {response_code}')
